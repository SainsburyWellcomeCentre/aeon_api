...
class TestFrames:
    ...
    def mock_capture(self, mocker, mock_frame):
        """Return a mock cv2.VideoCapture object and the constructor patch."""
...
class TestExport:
    ...
    def mock_writer(self, mocker):
        """Return a mock cv2.VideoWriter object and the constructor patch."""

        def factory(*, write_side_effect=None):
            instance = mocker.Mock()
            if write_side_effect is not None:
                instance.write.side_effect = write_side_effect
            constructor = mocker.patch("swc.aeon.io.video.cv2.VideoWriter", return_value=instance)
            return instance, constructor
    ...
    @pytest.mark.parametrize(
        ("n_frames", "write_calls", "release_calls"),
        [
            (1, 1, 1),
            (5, 5, 1),
        ],
        ids=["single frame", "multiple frames"],
    )
    def test_export_frames(self, mock_writer, mock_frame, fourcc, n_frames, write_calls, release_calls):
        """Test `export` with valid frames."""
        mock_writer_instance, mock_writer_constructor = mock_writer()
        fps = 30
        ...
        assert mock_writer_instance.write.call_count == write_calls
        assert mock_writer_instance.release.call_count == release_calls

    def test_export_error_handling(self, mock_writer, mock_frame):
        """Test that `export` releases the VideoWriter even when exception occurs."""
        mock_writer_instance, _ = mock_writer()
        mock_writer_instance.write.side_effect = RuntimeError("Write error")
        with pytest.raises(RuntimeError):
            video.export([mock_frame], "output.avi", fps=30)
